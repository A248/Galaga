...
class BeeSoul(AlienSoul):
    ...
    def initialize_entity(self, alienEntity) -> None:
        alienEntity.dance_stage = BeeSpiral(
            BeeBackAndForth(3), alienEntity.position, alienEntity.positionAtRest)

    def dance_entity(self, alienEntity) -> None:
        (positionDelta, newDance) = alienEntity.dance_stage.advance()
...
class DanceStage(object):
    ...
    def advance(self):
        (positionDelta, shouldCede) = self.advance_or_cede()
...
class BeeSpiral(DanceStage):
    def __init__(self, nextDance, startPosition, endPosition, timeToReach = 16):
